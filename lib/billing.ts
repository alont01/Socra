...
/** Duration of one session in hours (never negative, e.g. bad/reversed data). */
...
export function aggregateBilling(rows: SessionHoursRow[], rateUsd: number): ParentBilling[] {
  const byParent = new Map<string, { name: string; email: string; children: Map<string, { name: string; hours: number }> }>()

  for (const row of rows) {
    const hours = sessionHours(row.startedAt, row.endedAt)
    if (hours <= 0) continue

    if (!byParent.has(row.parentId)) {
      byParent.set(row.parentId, { name: row.parentName, email: row.parentEmail, children: new Map() })
    }
